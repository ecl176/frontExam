...
;!function(){
    ...
    Class.prototype.render = function(){
      ...
      lay(divFooter).html(function(){
        ...
        lay.each(options.btns, function(i, item){
          ...
          if(options.type !== 'date' && item === 'today') return ;
  ...
  Class.prototype.done = function(param, type){
    ...
    Class.prototype.choose = function(td){
      ...
      if(options.range){
        ...
        if(that.endState){ //重新选择
          ...
          if(options.type === 'date' && that.startState) {
            // setDateTime(true);
            that.setValue(that.parse()).remove().done();
          }
        } else { //选中开始
          ...
          setDateTime(); 
          that.startState = true;
          if(options.type === 'date' && that.endState) {
            // setDateTime(true);
            that.setValue(that.parse()).remove().done();
          }
        }
    ...
    Class.prototype.tool = function(btn, type){
      ...
      ,active = {
        ...
        ,confirm: function(){
          if(options.range){
            if(!that.endDate) return that.hint('请先选择日期范围');
            // 2019-6-12 add start 时间范围选择器中 设置默认值, 只选一次时间,报错无提示
            var elemBtn = lay(that.footer).find(ELEM_CONFIRM);
            var isOut;
            isOut = (that.newDate(that.startDate).getTime() > that.newDate(that.endDate).getTime());
            //如果不在有效日期内，直接禁用按钮，否则比较开始和结束日期
            (that.limit(null, that.startDate) || that.limit(null, that.endDate)) 
              ? elemBtn.addClass(DISABLED)
            : elemBtn[isOut ? 'addClass' : 'removeClass'](DISABLED);
            //是否异常提示
            if(isOut) {
            ...
            // 2019-6-12 add end 时间范围选择器中 设置默认值, 只选一次时间,报错无提示
    ...
    //日历图标与关闭图标切换
  Class.prototype.changeCloseIcon = function (value, $dom, className) {
    if(value !== "" && value !== null && value !== undefined) {
      $dom.removeClass(className).addClass("aidicon-close-line");
    } else {
      $dom.removeClass("aidicon-close-line").addClass(className);
    }
  }
    ...
    Class.prototype.events = function(){
      ...
      ,showEvent = function(elem, bind){
        elem.on(options.trigger, function(){
          ...
          if(options.type === 'date'){
            ...
            $(".laydate-footer-btns").css(cssObj);
            // $("body .laydate-footer-btns span").css("display", "none");
            // $("body .laydate-footer-btns span").css("display", "none");
            // $("body .laydate-footer-btns .laydate-btns-today").css("display", "block");
          }
      ...
      //时间选择器的鼠标移入移出事件
      var $dom, className;
      lay(".layout-date").on('mouseover' ,function () {
        var value = $(this).closest(".layout-date").find(".layout-date-value").val();
        var eventElem = options.eventElem;
        $dom = $(this).closest(".layout-date").find(eventElem);
        className = $dom.hasClass("aidicon-calendar")?'aidicon-calendar' : 'aidicon-clock';
        that.changeCloseIcon(value,$dom, className);
      }).on('mouseout',function () {
        that.changeCloseIcon('',$dom, className);
      });
      ...
      lay(document).on('click', function(e){
        ...
        || e.target === lay(options.closeStop)[0]){
          if(lay(e.target).hasClass("aidicon-close-line")) {
            that.tool(this, 'clear');
          }
          return;
        }
        that.remove();
