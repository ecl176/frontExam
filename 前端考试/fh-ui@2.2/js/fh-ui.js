...
require("./bootstrap/areaSelect");
},{"./bootstrap/affix":4,"./bootstrap/alert":5,"./bootstrap/areaMultiSelect":6,"./bootstrap/areaSelect":7,"./bootstrap/button":8,"./bootstrap/carousel":9,"./bootstrap/cascader":10,"./bootstrap/collapse":11,"./bootstrap/dropdown":13,"./bootstrap/dropdown-select":12,"./bootstrap/filter":14,"./bootstrap/global":15,"./bootstrap/input-groups":16,"./bootstrap/leftNav":17,"./bootstrap/modal":18,"./bootstrap/navDrawer":19,"./bootstrap/navTop":20,"./bootstrap/popover":21,"./bootstrap/position":22,"./bootstrap/rate":23,"./bootstrap/scrollspy":24,"./bootstrap/step":25,"./bootstrap/tab":26,"./bootstrap/timeline":27,"./bootstrap/tooltip":28,"./bootstrap/transition":29}],4:[function(require,module,exports){
/* ========================================================================
...
+function ($) {
  ...
  Affix.prototype.getState = function (scrollHeight, height, offsetTop, offsetBottom) {
    ...
    if (offsetTop != null && String(this.affixed) === 'top') return scrollTop < offsetTop ? 'top' : false;

    if (String(this.affixed) === 'bottom') {
  ...
  Affix.prototype.checkPosition = function () {
    ...
    if (this.affixed != affix) {
      ...
      this.affixed = affix;
      this.unpin = String(affix) === 'bottom' ? this.getPinnedOffset() : null;
...
+function ($) {
  ...
  AreaMultiSelect.prototype.templeteRoot = function () {
    ...
    root += '<button type="button" class="btn submit btn-primary btn-sm"  disabled="disabled">确定</button>';
    ...
    dataList.forEach(function (item) {
      ...
      if (item.children.length > 0) {
        ...
        item.children.forEach(function (childItem) {
          ...
          cityStr += '<label class="checkbox-inline checkbox-beauty">';
          cityStr += '<input type="checkbox">';
          cityStr += '<span class="text-label" node-id="' + childItem.id + '"></span>';
          cityStr += '</label>';
          if(childItem.name.length > 5) {
            cityStr += '<label class="city-name showTips" data-toggle="popover" data-placement="top" data-content="' + childItem.name + '">';
          } else {
            cityStr += '<label class="city-name">';
          }

          cityStr += '<span>'+childItem.name+'</span>';

          if (childItem.children.length > 0) {
            cityStr += '<span class="caret"></span></label>';
            cityStr += '<div class="select-content-child">';
            if (_self.options.mode === "strict") {
              cityStr += '<div class="select-content-child-title">选择区<i class="aidicon aidicon-close"></i></div>';
            }
  ...
  AreaMultiSelect.prototype.selectChild = function ($element) {
    ...
    $element.parents(".content-item").find(".city-name span:first-child span").remove();
    $element.parents(".select-content-child").siblings(".checkbox-beauty").removeClass("part-select");
    if (selectNumber > 0) {
      $element.parents(".content-item").children(".city-name").children("span:first-child").append("<span>(" + selectNumber + ")</span>");
  ...
  AreaMultiSelect.prototype.returnSelect = function () {
    ...
    //处理选中的省数据
    _self.$AreaMultiSelect.find(".select-content-center-in-left input:checkbox:checked").each(function () {
    ...
    _self.$AreaMultiSelect.find(".content-item>label input:checkbox:checked").each(function () {
    ...
    _self.$AreaMultiSelect.find(".content-item .select-content-child input:checkbox:checked").each(function () {
  ...
  AreaMultiSelect.prototype.setState = function (_this, isBottom) {
    ...
    if(isBottom) {
      ...
      $doms = _this.parents(".select-content-center-in").find('.select-content-center-in-right');
    }

    $doms.each(function () {
      checkboxLen = $(this).find(".content-item:not('.area-disabled') label:not('.area-disabled') input:checkbox").length;
      checkedLen = $(this).find(".content-item:not('.area-disabled') label:not('.area-disabled') input:checkbox:checked").length;
    ...
    //确定按钮是否禁用
    var ele = this.$element.length > 0 && this.$element[0].id !== "" ? "#"+this.$element[0].id: this.$element[0].className.split(" ").join(".");
    if(allCheckedLen > 0) {
      $(ele+" .select-content-bottom .submit").removeAttr("disabled");
    }
    // else {
    //   $(ele+" .select-content-bottom .submit").attr("disabled", "disabled");
    // }
  }
  ...
  AreaMultiSelect.prototype.initEvents = function () {
    ...
    _self.$element.on(_self.options.trigger, '.area-select-content, .area-select-open', function (e) {
      e.stopPropagation();
      if(_self.$element.find('.area-select-item').length > 0 && e.currentTarget.className.split(" ").indexOf("area-select-content") >= 0) {
        return ;
      }
      var contentHeight = _self.$element.find(".area-select-content").outerHeight();
      _self.$AreaMultiSelect.css("top", contentHeight).find(".select-content-outer").show(0, function () {
    ...
    //左侧全选(省份)
    _self.$AreaMultiSelect.on('click', '.select-content-center-in-left .checkbox-inline', function (e) {
      e.stopPropagation();
      var _this = $(this);
    ...
    //右侧选择(城市)
    _self.$AreaMultiSelect.on('click', '.select-content-center-in-right .content-item:not(".area-disabled") .checkbox-inline', function (e) {
      e.stopPropagation();
      var _this = $(this);
    ...
    if (_self.options.mode === "default") {
      _self.$AreaMultiSelect.on('click', '.select-content', function (e) {
        e.stopPropagation();
        if ((String(e.target.nodeName) === "SPAN" || String(e.target.nodeName) === "INPUT" || String(e.target.nodeName) === "LABEL") || (String(e.target.nodeName) === "DIV" && $(e.target).hasClass("select-content-child-top"))) return;
        _self.$AreaMultiSelect.find(".select-content-center-in-right .content-item .caret").each(function () {
          if ($(this).hasClass("open")) {
            $(this).parent().siblings(".select-content-child").find('.select-content-child-bottom .btn.submit').trigger("click");
          }
        });
      });
    }
    ...
    //打开选择子地区
    _self.$AreaMultiSelect.on('click', '.select-content-center-in-right .content-item .city-name', function (e) {
      e.stopPropagation();
      if (_self.options.mode === "strict") {//严格模式
        if ($(this).find(".caret").hasClass("open")) return;
        _self.$AreaMultiSelect.find(".select-content-center-in-right .content-item .caret").each(function () {
          if ($(this).hasClass("open")) {
            $(this).parent().siblings(".select-content-child").find('.select-content-child-bottom .btn.cancel').trigger("click");
          }
        });
      } else {
    ...
    //区域选择
    _self.$AreaMultiSelect.on('click', '.select-content-child .checkbox-inline:not(".area-disabled")', function (e) {
      e.stopPropagation();
    ...
    //区域选择中的确认
    _self.$AreaMultiSelect.on('click', '.select-content-child-bottom .btn.submit', function (e) {
      e.stopPropagation();
      _self.setState($(this));
      $(this).parents(".select-content-child").hide();
      $(this).parents(".select-content-child").siblings(".city-name").find(".caret").removeClass("open");
    });

    //区域选择中的取消
    _self.$AreaMultiSelect.on('click', '.select-content-child-bottom .btn.cancel', function (e) {
      e.stopPropagation();
    ...
    //区域选择中的关闭
    _self.$AreaMultiSelect.on('click', '.select-content-child-title > .aidicon', function (e) {
      e.stopPropagation();
      $(this).parents(".select-content-child").find(".select-content-child-bottom .btn.cancel").trigger("click");
    });

    //显示框中地区删除
    _self.$element.on('click', '.area-select-item .label .aidicon', function (e) {
      e.stopPropagation();
      var $id = $(this).parent().attr("area-id");


      _self.$AreaMultiSelect.find("[node-id='" + $id + "']").parent(".checkbox-inline").trigger("click");
      _self.selectChild(_self.$AreaMultiSelect.find("[node-id='" + $id + "']"));
      _self.$AreaMultiSelect.find("[node-id='" + $id + "']").parents(".select-content-child").find(".select-content-child-bottom .submit").trigger('click');

      var startEvent = $.Event('submit.bs.areaMultiSelect', _self.returnSelect());
      _self.$element.trigger(startEvent);

      _self.options.selectList = startEvent.selectInfo.selectIdList.all;
      _self.setSelect(_self.options.selectList);
      ...
      var contentHeight = _self.$element.find(".area-select-content").outerHeight();
      _self.$AreaMultiSelect.css("top", contentHeight);
    });

    //省略图标鼠标移入展开
    _self.$element.on('mouseenter', '.area-select-icon .area-ellipse', function (e) {
      e.stopPropagation();
      if (_self.$element.find(".area-select-content .area-select-item").length > 0 && _self.$element.find(".area-select-icon .area-ellipse").length > 0) {
        ...
        _self.$element.find(".area-select-item .province:nth-child(n+2)").before('<br />');
        var contentHeight = _self.$element.find(".area-select-content").outerHeight();
        _self.$AreaMultiSelect.css("top", contentHeight);
      }
    });

    //鼠标移出area-select-content热区收缩
    _self.$element.on('mouseleave', '.area-select-content', function (e) {
      e.stopPropagation();
      if (_self.$element.find(".area-select-content .area-select-item").length > 0 && _self.$element.find(".area-select-content").hasClass("open")) {
          if (_self.$element.find(".area-select-item").height() > 34) {//內容超出一行添加省略
            _self.$element.find(".area-select-icon").empty().prepend("<span class='area-ellipse'>...</span><span class='area-select-open'>编辑</span>");
          ...
          _self.$element.find(".area-select-item br").remove();
        var contentHeight = _self.$element.find(".area-select-content").outerHeight();
        _self.$AreaMultiSelect.css("top", contentHeight);
      }
    });
    ...
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .btn.submit', function (e) {
      ...
      $(this).parents(".select-content").find(".select-content-child").hide().siblings(".city-name").find(".caret").removeClass("open");
    });
    ...
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .btn.clear', function (e) {
      ...
      _self.$AreaMultiSelect.find(".city-name span:first-child span").remove();
    ...
    $left.each(function (index, left) {
      ...
      $right.each(function (index, right) {
        if ($(right).find(".select-content-child input:checkbox:checked").length > 0 || $(right).find("> .checkbox-inline input:checkbox:checked").length > 0) {
          ...
          cityObj.name = $(right).find(" > .city-name span:first-child").html().split("<span>")[0];
          cityObj.id = $(right).find(" > .checkbox-beauty span").attr("node-id");
  ...
  AreaMultiSelect.prototype.selectedText = function (list) {
    var $dom = "";
    if(list.length > 0) {
      $dom += '<div class="area-select-item">';
      list.forEach(function (item) {
        if (item.child && item.child.length > 0) {
          item.child.forEach(function (childItem) {
            $dom += '<span class="province">' + item.name + ' / ' + childItem.name + '</span>';
      ...
      $dom += '</div>';
      $dom += '<span class="area-select-icon"><span class="area-select-open">编辑</span></span>';
    } else {
      $dom += '<span class="area-select-icon"><span class="area-select-open">选择</span></span>';
    }
    return $dom;
  }
  ...
  AreaMultiSelect.prototype.appendContent = function ($selectDom) {
    ...
    if (_self.$element.find(".area-select-content.open").length === 0) {
      if (_self.$element.find(".area-select-item").length > 0) {
        if(_self.$element.find(".area-select-item").height() > 34) {
          _self.$element.find(".area-select-icon").prepend("<span class='area-ellipse'>...</span>");
    ...
    } else if(_self.$element.find(".area-select-content").hasClass('open')) {
      _self.$element.find(".area-select-item .province:nth-child(n+2)").before('<br />');
    }
    if (_self.$element.find(".area-select-item").length === 0 || _self.$element.find(".select-content-center .content-item:not('.area-disabled') input:checked").length === 0) {
      _self.$element.find(".area-select-content").append(_self.placeholder).removeClass("open ellipse");
      _self.$element.find(".area-select-icon .area-ellipse").remove();
  ...
  AreaMultiSelect.prototype.setSelect = function (selectList, isInit) {//设置选中
    ...
    var $domList = {};
    _self.$AreaMultiSelect.find(".checkbox-beauty").each(function () {
...
+function ($) {
  ...
  // areaSelect CLASS DEFINITION
  // =========================

  var allCities = ['北京|beijing|bj', '上海|shanghai|sh', '天津|tianjin|tj', '重庆|chongqing|cq', '阿克苏|akesu|aks', '安宁|anning|an', '安庆|anqing|aq', '鞍山|anshan|as', '安顺|anshun|as', '安阳|anyang|ay',
    '白城|baicheng|bc', '白山|baishan|bs', '白银|baiyin|by', '蚌埠|bengbu|bb', '保定|baoding|bd', '宝鸡|baoji|bj', '保山|baoshan|bs', '巴中|bazhong|bz', '北海|beihai|bh', '本溪|benxi|bx',
    '滨州|binzhou|bz', '博乐|bole|bl', '亳州|bozhou|bz', '沧州|cangzhou|cz', '常德|changde|cd', '昌吉|changji|cj', '常熟|changshu|cs', '常州|changzhou|cz', '巢湖|chaohu|ch', '朝阳|chaoyang|cy',
    '潮州|chaozhou|cz', '承德|chengde|cd', '成都|chengdu|cd', '城固|chenggu|cg', '郴州|chenzhou|cz', '赤壁|chibi|cb', '赤水|chishui|cs', '池州|chizhou|cz', '崇左|chongzuo|cz', '楚雄|chuxiong|cx',
    '滁州|chuzhou|cz', '慈溪|cixi|cx', '从化|conghua|ch', '大理|dali|dl', '大连|dalian|dl', '丹东|dandong|dd', '丹阳|danyang|dy', '大庆|daqing|dq', '大同|datong|dt', '达州|dazhou|dz',
    '德阳|deyang|dy', '德州|dezhou|dz', '东莞|dongguan|dg', '东阳|dongyang|dy', '东营|dongying|dy', '都匀|duyun|dy', '敦化|dunhua|dh', '鄂尔多斯|eerduosi|eeds', '恩施|enshi|es', '防城港|fangchenggang|fcg',
    '肥城|feicheng|fc', '奉化|fenghua|fh', '抚顺|fushun|fs', '阜新|fuxin|fx', '阜阳|fuyang|fy', '富阳|fuyang|fy', '福州|fuzhou|fz', '抚州|fuzhou|fz', '赣榆|ganyu|gy', '赣州|ganzhou|gz',
    '高明|gaoming|gy', '高邮|gaoyou|gy','格尔木|geermu|gem', '个旧|gejiu|gj', '巩义|gongyi|gy', '广安|guangan|ga', '广元|guangyuan|gy', '广州|guangzhou|gz', '古包头|gubaotou|gbt', '贵港|guigang|gg',
    '桂林|guilin|gl', '贵阳|guiyang|gy', '固原|guyuan|gy', '哈尔滨|haerbin|heb', '海城|haicheng|hc', '海口|haikou|hk', '海门|haimen|hm', '海宁|haining|hn', '哈密|hami|hm', '邯郸|handan|hd',
    '杭州|hangzhou|hz', '汉中|hanzhong|hz', '鹤壁|hebi|hb', '合肥|hefei|hf', '衡水|hengshui|hs', '衡阳|hengyang|hy', '和田|hetian|ht', '河源|heyuan|hy', '菏泽|heze|hz', '花都|huadu|hd',
    '淮安|huaian|ha', '淮北|huaibei|hb', '怀化|huaihua|hh', '淮南|huainan|hn', '黄冈|huanggang|hg', '黄山|huangshan|hs', '黄石|huangshi|hs', '呼和浩特|huhehaote|hhht', '惠州|huizhou|hz', '葫芦岛|huludao|hld',
    '湖州|huzhou|hz', '佳木斯|jiamusi|jms', '吉安|jian|ja', '江都|jaingdu|jd', '江门|jiangmen|jm', '江阴|jiangyin|jy', '胶南|jiaonan|jn', '胶州|jiaozhou|jz', '焦作|jiaozuo|jz', '嘉善|jiashan|js',
    '嘉兴|jiaxing|jx', '介休|jiexiu|jx', '吉林|jilin|jl', '即墨|jimo|jm', '济南|jinan|jn', '晋城|jincheng|jc', '景德镇|jingdezhen|jdz', '景洪|jinghong|jh', '靖江|jingjiang|jj', '荆门|jingmen|jm',
    '荆州|jingzhou|jz', '金华|jinhua|jh', '集宁|jining|jn', '济宁|jining|jn', '晋江|jinjiang|jj', '金坛|jintan|jt', '晋中|jinzhong|jz', '锦州|jinzhou|jz', '吉首|jishou|js', '九江|jiujiang|jj',
    '酒泉|jiuquan|jq', '鸡西|jixi|jx', '济源|jiyuan|jy', '句容|jurong|jr', '开封|kaifeng|kf', '凯里|kaili|kl', '开平|kaiping|kp', '开远|kaiyuan|ky', '咯什|kashen|ks', '克拉玛依|kelamayi|klmy',
    '库尔勒|kuerle|kel', '奎屯|kuitun|kt', '昆明|kunming|km', '昆山|kunshan|ks', '来宾|laibin|lb', '莱芜|laiwu|lw', '莱西|laixi|lx', '莱州|laizhou|lz', '廊坊|langfang|lf', '兰州|lanzhou|lz',
    '拉萨|lasa|ls', '乐山|leshan|ls', '连云港|lianyungang|lyg', '聊城|liaocheng|lc', '辽阳|liaoyang|ly', '辽源|liaoyuan|ly', '丽江|lijaing|lj', '临安|linan|la', '临沧|lincang|lc', '临汾|linfen|lf',
    '灵宝|lingbao|lb', '临河|linhe|lh', '临夏|linxia|lx', '临沂|linyi|ly', '丽水|lishui|ls', '六安|luan|la', '六盘水|liupanshui|lps', '柳州|liuzhou|lz', '溧阳|liyang|ly', '龙海|longhai|lh',
    '龙岩|longyan|ly', '娄底|loudi|ld', '漯河|luohe|lh', '洛阳|luoyang|ly', '潞西|luxi|lx', '泸州|luzhou|lz',  '吕梁|lvliang|ll', '旅顺|lvshun|ls', '马鞍山|maanshan|mas', '茂名|maoming|mm',
    '梅河口|meihekou|mhk', '眉山|meishan|ms', '梅州|meizhou|mz', '勉县|mianxian|mx', '绵阳|mianyang|my', '牡丹江|mudanjiang|mdj', '南安|nanan|na', '南昌|nanchang|nc', '南充|nanchong|nc', '南京|nanjing|nj',
    '南宁|nanning|nn', '南平|nanping|np', '南通|nantong|nt', '南阳|nanyang|ny', '内江|neijiang|nj', '宁波|ningbo|nb', '宁德|ningde|nd', '盘锦|panjin|pj', '攀枝花|panzhihua|pzh', '蓬莱|penglai|pl',
    '平顶山|pingdingshan|pds', '平度|pingdu|pd', '平湖|pinghu|ph', '平凉|pingliang|pl', '萍乡|pingxiang|px', '普兰店|pulandian|pld', '普宁|puning|pn', '莆田|putian|pt', '濮阳|puyang|py', '黔南|qiannan|qn',
    '启东|qidong|qd', '青岛|qingdao|qd', '庆阳|qingyang|qy', '清远|qingyuan|qy', '青州|qingzhou|qz', '秦皇岛|qinhuangdao|qhd', '钦州|qinzhou|qz', '琼海|qionghai|qh', '齐齐哈尔|qiqihaer|qqhe', '泉州|quanzhou|qz',
    '曲靖|qujing|qj', '衢州|quzhou|qz', '日喀则|rikaze|rkz', '日照|rizhao|rz', '荣成|rongcheng|rc', '如皋|rugao|rg', '瑞安|ruian|ra', '乳山|rushan|rs', '三门峡|sanmenxia|smx', '三明|sanming|sm',
    '三亚|sanya|sy', '厦门|xiamen|xm', '佛山|foshan|fs', '商洛|shangluo|sl', '商丘|shangqiu|sq', '上饶|shangrao|sr', '上虞|shangyu|sy', '汕头|shantou|st', '安康|ankang|ak', '韶关|shaoguan|sg',
    '绍兴|shaoxing|sx', '邵阳|shaoyang|sy', '沈阳|shenyang|sy', '深圳|shenzhen|sz', '石河子|shihezi|shz', '石家庄|shijiazhuang|sjz', '石林|shilin|sl', '石狮|shishi|ss', '十堰|shiyan|sy', '寿光|shouguang|sg',
    '双鸭山|shuangyashan|sys', '朔州|shuozhou|sz', '沭阳|shuyang|sy', '思茅|simao|sm', '四平|siping|sp', '松原|songyuan|sy', '遂宁|suining|sn', '随州|suizhou|sz', '苏州|suzhou|sz', '塔城|tacheng|tc',
    '泰安|taian|ta', '太仓|taicang|tc', '泰兴|taixing|tx', '太原|taiyuan|ty', '泰州|taizhou|tz', '台州|taizhou|tz', '唐山|tangshan|ts', '腾冲|tengchong|tc', '滕州|tengzhou|tz', '天门|tianmen|tm',
    '天水|tianshui|ts', '铁岭|tieling|tl', '铜川|tongchuan|tc', '通辽|tongliao|tl', '铜陵|tongling|tl', '桐庐|tonglu|tl', '铜仁|tongren|tr', '通州|tongzhou|tz', '通化|tonghua|th', '吐鲁番|tulufan|tlf',
    '瓦房店|wafangdian|wfd', '潍坊|weifang|wf', '威海|weihai|wh', '渭南|weinan|wn', '文登|wendeng|wd', '温岭|wenling|wl', '温州|wenzhou|wz', '乌海|wuhai|wh', '武汉|wuhan|wh', '芜湖|wuhu|wh',
    '吴江|wujaing|wj', '乌兰浩特|wulanhaote|wlht', '武威|wuwei|ww', '无锡|wuxi|wx', '梧州|wuzhou|wz', '西安|xian|xa', '项城|xiangcheng|xc', '襄樊|xiangfan|xf', '香格里拉|xianggelila|xgll', '象山|xiangshan|xs',
    '湘潭|xiangtan|xt', '湘乡|xiangxiang|xx', '咸宁|xianning|xn', '仙桃|xiantao|xt', '咸阳|xianyang|xy', '西藏|xizang|xz', '西昌|xichang|xc', '邢台|xingtai|xt', '兴义|xingyi|xy', '西宁|xinign|xn',
    '新乡|xinxiang|xx', '信阳|xinyang|xy', '新余|xinyu|xy', '忻州|xinzhou|xz', '宿迁|suqian|sq', '宿豫|suyu|sy', '宿州|suzhou|sz', '宣城|xuancheng|xc', '许昌|xuchang|xc', '徐州|xuzhou|xz',
    '雅安|yaan|ya', '牙克石|yakeshi|yks', '延安|yanan|ya', '延边|yanbian|yb', '盐城|yancheng|yc', '阳江|yangjiang|yj', '阳泉|yangquan|yq', '扬州|yangzhou|yz', '延吉|yanji|yj', '烟台|yantai|yt',
    '兖州|yanzhou|yz', '宜宾|yibin|yb', '宜昌|yichang|yc', '宜春|yichun|yc', '伊春|yichun|yc', '伊犁|yili|yl', '银川|yinchuan|yc', '营口|yingkou|yk', '鹰潭|yingtan|yt', '伊宁|yining|yn',
    '义乌|yiwu|yw', '宜兴|yixing|yx', '益阳|yiyang|yy', '永康|yongkang|yk', '永州|yongzhou|yz', '岳阳|yueyang|yy', '玉环|yuhuan|yh', '榆林|yulin|yl', '玉林|yulin|yl', '运城|yuncheng|yc',
    '玉溪|yuxi|yx', '余姚|yuyao|yy', '枣庄|zaozhuang|zz', '增城|zengcheng|zc', '长春|changchun|cc', '长海|changhai|ch', '张家港|zhangjiagang|zjg', '张家界|zhangjiajie|zjj', '张家口|zhangjiakou|zjk', '长乐|changle|cl',
    '章丘|zhangqiu|zq', '长沙|changsha|cs', '张掖|zhangye|zy', '长治|changzhi|cz', '漳州|zhangzhou|zz', '湛江|zhanjiang|zj', '肇东|zhaodong|zd', '肇庆|zhaoqing|zq', '昭通|zhaotong|zt', '郑州|zhengzhou|zz',
    '镇江|zhenjiang|zj', '中山|zhongshan|zs', '周口|zhoukou|zk', '舟山|zhoushan|zs', '诸城|zhucheng|zc', '珠海|zhuhai|zh', '诸暨|zhuji|zj', '驻马店|zhumadian|zmd', '株洲|zhuzhou|zz', '淄博|zibo|zb',
    '自贡|zigong|zg', '遵义|zunyi|zy', '乌鲁木齐|wulumuqi|wlmq', '福清|fuqing|fq', '鄂州|ezhou|ez', '包头|baotou|bt', '萧山|xiaoshan|xs', '宣化|xuanhua|xh', '江油|jaingyou|jy', '资阳|ziyang|zy',
    '辛集|xinji|xj', "佛山|foshan|fs", "万州|wanzhou|wz", '邹城|zoucheng|zc', '姜堰|jiangyan|jy', '湘阴|xiangyin|xy', '松江|songjiang|sj', '七台河|qitaihe|qth', '醴陵|liling|ll', '涪陵|fuling|fl',
    '公主岭|gongzhuling|gzl', '歙县|shexian|sx', '兴化|xinghua|xh', '新民|xinmin|xm', '庄河|zhuanghe|zh', '东港|donggang|dg', '凤城|fengcheng|fc', '凌海|linghai|lh', '北镇|beizhen|bz', '大石桥|dashiqiao|dsq',
    '盖州|gaizhou|gz', '灯塔|dengta|dt', '调兵山|diaobingshan|dbs', '开原|kaiyuan|ky', '北票|beipiao|bp', '兴城|xingcheng|xc', '鹤岗|hegang|hg', '黑河|heihe|hh', '绥化|suihua|sh', '大兴安岭地区|daxinganlingdiqu|dxaldq',
    '嘉峪关|jiayuguan|jyg', '金昌|jinchang|jc', '定西|dingxi|dx', '陇南|longnan|ln', '甘南|gannan|gn', '玉树|yushu|ys', '果洛|guoluo|gl', '海东|haidong|hd', '海西|haixi|hx', '海南|hainan|hn',
    '海北|haibei|hb', '建德|jiande|jd', '乐青|leqing|lq', '嵊州|shengzhou|sz', '桐乡|tongxiang|tx', '江山|jiangshan|js', '龙泉|longquan|lq', '永康|yongkang|yk', '汕尾|shanwei|sw', '南雄|nanxiong|nx',
    '乐昌|lechang|lc', '英德|yingde|yd', '连州|lianzhou|lz', '台山|taishan|ts', '鹤山|heshan|hs', '恩平|enping|ep', '廉江|lianjiang|lj', '雷州|leizhou|lz', '吴川|wuchaun|wc', '高州|gaozhou|gz',
    '化州|huazhou|hz', '信宜|xinyi|xy', '高要|gaoyao|gy', '四会|sihui|sh', '兴宁|xingning|xn', '陆丰|lufeng|lf', '阳春|yangchun|yc', '普宁|puning|pn', '罗定|luoding|ld', '红河|honghe|hh',
    '迪庆|diqing|dq', '文山|wenshan|ws', '西双版纳|xishuangbanna|xsbn', '德宏|dehong|dh', '怒江|nujiang|nj', '台北|taibei|tb', '新北|xinbei|xb', '桃园|taoyuan|ty', '台中|taizhong|tz', '台南|tainan|tn',
    '高雄|gaoxiong|gx', '五指山|wuzhishan|wzs', '儋州|danzhou|dz', '万宁|wanning|wn', '甘孜|ganzi|gz', '阿坝|aba|ab', '毕节|bijie|bj', '桐城|tongcheng|tc', '界首|jieshou|js', '天长|tianchang|tc',
    '明光|mingguang|mg', '宁国|ningguo|ng'
  ];

  var regEx = /^([\u4E00-\u9FA5\uf900-\ufa2d]+)\|(\w+)\|(\w)\w*$/i, // 匹配汉字，拼音
    // regExChiese = /([\u4E00-\u9FA5\uf900-\ufa2d]+)/, // 只匹配拼音
    reg_ae = /^[a-e]$/i, // 匹配首字母为 a-e
    reg_fk = /^[f-k]$/i, // 匹配首字母为 f-k
    reg_lp = /^[l-p]$/i, // 匹配首字母为 l-p
    reg_qu = /^[q-u]$/i, // 匹配首字母为 q-u
    reg_vz = /^[v-z]$/i, // 匹配首字母为 v-z
    reg_a = /^[a]$/i, // 匹配首字母为 a
    reg_b = /^[b]$/i, // 匹配首字母为 b
    reg_c = /^[c]$/i, // 匹配首字母为 c
    reg_d = /^[d]$/i, // 匹配首字母为 d
  ...
  var AreaSelect = function (element,options) {
    this.element = element;
    this.options = $.extend({}, AreaSelect.DEFAULTS, options);
  };

  AreaSelect.VERSION = '1.0.0';

  AreaSelect.DEFAULTS = {
    "type": "tabs",
    "hotCities": ['北京', '上海', '天津', '重庆', '成都', '大连','青岛', '西安', '南京', '苏州', '杭州', '镇江', '厦门',
      '武汉', '长沙', '昆明', '深圳', '广州', '济南', '新疆','西藏', '合肥', '贵阳', '长春', '石家庄', '连云港']
  };

  AreaSelect.prototype.cityFormat = function() {
    for (var i = 0, len = allCities.length; i < len; i++) {
      ...
      if(this.options.type === 'tabs') {
        if (reg_ae.test(first)) {
          ltPart = 'ABCDE';
        } else if (reg_fk.test(first)) {
          ltPart = 'FGHIJK';
        } else if (reg_lp.test(first)) {
          ltPart = 'LMNOP';
        } else if (reg_qu.test(first)) {
          ltPart = 'QRSTU';
        } else if (reg_vz.test(first)) {
          ltPart = 'VWXYZ';
        }
        if(city[ltPart][first]) {
          city[ltPart][first].push(en);
        } else {
          city[ltPart][first] = [];
          city[ltPart][first].push(en);
        }
        // city[ltPart][first] ? city[ltPart][first].push(en) : (city[ltPart][first] = [], city[ltPart][first].push(en));
        city.hot['hot'] = this.options.hotCities;
      } else if(this.options.type === 'tile') {
        if (reg_a.test(first)) {
          ltPart = 'A';
          ...
          ltPart = 'T';
        } else if (reg_u.test(first)) {
          ltPart = 'U';
        } else if (reg_v.test(first)) {
          ltPart = 'V';
        ...
        // cityTile[ltPart] ? cityTile[ltPart].push(en) : (cityTile[ltPart] = [], cityTile[ltPart].push(en));
        if(cityTile[ltPart]) {
          cityTile[ltPart].push(en);
        } else {
          cityTile[ltPart] = [];
          cityTile[ltPart].push(en);
        }
        cityTile.hot = this.options.hotCities;
      }
    }
  };

  //tabs切式
  AreaSelect.prototype.createItem = function() {
    if(this.element.find(".city-select").length > 0) {
      return;
    }
    var template = '<div class="city-select"><div class="nav-card-tabs-wrapper-line">' +
      '    <ul class="nav nav-card-tabs city-select-head">' +
      '        <li class="active"><a href="javascript:void(0)">热门城市</a></li>' +
      '        <li><a href="javascript:void(0)">ABCDE</a></li>' +
      '        <li><a href="javascript:void(0)">FGHIJK</a></li>' +
      '        <li><a href="javascript:void(0)">LMNOP</a></li>' +
      '        <li><a href="javascript:void(0)">QRSTU</a></li>' +
      '        <li><a href="javascript:void(0)">VWXYZ</a></li>' +
      '    </ul>' +
    ...
    for (var group in city) {
      ...
      itemKey.sort();
      var itemBox = $('<div class="city_item">');
      ...
      for (var i = 0, iLen = itemKey.length; i < iLen; i++) {
        ...
        for (var j = 0, jLen = city[group][itemKey[i]].length; j < jLen; j++) {
            str += '<div class="radio" title="'+city[group][itemKey[i]][j]+'">' +
            '<label class="radio-card">' +
            '<input type="radio" name="city-radio" value="'+city[group][itemKey[i]][j]+'">' +
            '<span class="text-label">'+city[group][itemKey[i]][j]+'</span>'+
            '</label>'+
            '</div>';
        }
      ...
      this.element.find('.city-content').append(itemBox);
      this.element.find('.city-select .hot').addClass('active');
    }
  };

  //平铺式
  AreaSelect.prototype.createTileItem = function() {
    if(this.element.find(".city-select-tile").length > 0) {return;}
    var template = '<div class="city-select-tile filter">' +
      '<div class="filter-body">' +
      '  <div class="form-group clearfix hotCities city-content">' +
      '    <label class="labels">热门城市</label>' +
      '   </div>' +
      '  <div class="form-group clearfix initial">' +
      '    <label class="labels">首字母选择</label>' +
      '   </div>' +
      '</div></div>';
    this.element.append(template);

    for (var group in cityTile) {
        if(group === 'hot') {
          var hotStr = '';
          for (var j = 0; j < cityTile[group].length; j++) {
            var tempClass = cityTile[group][j].length >= 5 ? "city-over" : "";
            hotStr += '<label class="radio-inline radio-card ' +tempClass+ '" title="'+cityTile[group][j]+'">' +
              '<input type="radio" name="city-radio-select" value="' + cityTile[group][j] + '">' +
              '<span class="text-label">' + cityTile[group][j] + '</span>' +
              '</label>';
          }
          this.element.find(".hotCities").append(hotStr);
        } else {
          var initialStr = '', len = cityTile[group].length;
          initialStr += '<div class="initial-content initial-'+group+'">' +
            '<span class="text-label">' + group + '</span>';
          if(len > 0) {
            initialStr += '<div class="city-content city-' + group + '">' +
              '<div class="arrow-top"></div>'
            ;

            for (var k = 0; k < len; k++) {
              var temp = cityTile[group][k].length >= 5 ? "city-over" : "";
              initialStr += '<label class="radio-inline radio-card '+temp+'" title="'+cityTile[group][k]+'">' +
                '<input type="radio" name="city-radio-select" value="' + cityTile[group][k] + '">' +
                '<span class="text-label">' + cityTile[group][k] + '</span>' +
                '</label>';
            }
            initialStr += '</div></div>';
          }
          this.element.find(".initial").append(initialStr);
        }
      }
  };

  //初始化地区 - 禁用
  AreaSelect.prototype.initAreaDisabled = function() {
    var disabledList = this.options.disabledCities,
    ele = this.element.length > 0 && this.element[0].id !== "" ? "#"+this.element[0].id: this.element[0].className.split(" ").join(".");

    if(disabledList) {
      for (var i = 0; i < disabledList.length; i++) {
        $(ele + " .city-content .radio-card [value='" + disabledList[i] + "']").parent().addClass("area-disabled");
      }
    }
  }

  AreaSelect.prototype.citySelectEvent = function() {
    var self = this;
    //tabs头部点击事件
    self.element.find('.city-select-head').off('click', 'li a').on('click', 'li a', function(e) {
      var current = $(e.target).parent(),
        index = current.index();

      self.element.find('.city-select-head li').eq(index).addClass('active').siblings().removeClass('active');
      self.element.find('.city_item').eq(index).addClass('active').siblings().removeClass('active');
      self.element.find('.city-content').scrollTop(0);
    });

    //tabs内容点击
    self.element.find('.city_item dd').off('click', 'span').on('click', 'span', function (e) {
      e.stopPropagation();
      self.element.find(".city-name").text(($(e.target).text())).addClass("selected-city-name");
      self.element.find('.city-select').removeClass("open").hide();
      self.element.find(".aidicon").removeClass("aidicon-chevron-up").addClass("aidicon-chevron-down");
    });

    //平铺点击
    self.element.find(".initial").off("click", '> .initial-content').on('click', '> .initial-content', function (e) {
      e.stopPropagation();
      if($(this).hasClass("active")) {
        $(this).removeClass("active");
      } else {
          // var index = $(this).index(),
          // initialLeft = index * 34 - 8 - 13 -6;
          var initialLeft = $(this).position().left - self.element.find(".initial .labels").outerWidth() - 9,
          cityContentLeft = $(this).find(".city-content").width();
        if (initialLeft >= cityContentLeft) {
          $(this).find(".city-content").css("left", (initialLeft - cityContentLeft + 85 + 26) + 'px').find(".arrow-top").css("left", (cityContentLeft - 26) + 'px');
        } else {
          $(this).find(".arrow-top").css("left", initialLeft + "px");
        }
        $(this).addClass("active").siblings(".initial-content").removeClass("active");
      }
    });
  };

  AreaSelect.prototype.stopPropagation = function() {
    var self = this;
    //阻止事件冒泡
    self.element.find('.city-select').on('click', stopPropagation);
    self.element.on('click', stopPropagation);
    //页面点击 隐藏
  ...
  AreaSelect.prototype.init = function () {
    var _self = this;
    _self.cityFormat();
    _self.options.type !== 'tile' ? _self.createItem() : _self.createTileItem();
    _self.initAreaDisabled();
    _self.citySelectEvent();
    _self.stopPropagation();
  };
  ...
  function Plugin(option) {
    ...
    if(element.children().length === 0) {
      var str = '<span class="city-name">请选择...</span>' +
        '<span class="aidicon aidicon-chevron-down"></span>';
      element.append(str).addClass("area-select-tabs");
    }
    if(option && option.type === 'tile') {
      element.empty().removeClass().addClass("area-select-initial");
      areaSelect = areaSelect && areaSelect.element === element ? areaSelect : new AreaSelect(element, option);
      areaSelect.init();
    }
    element.on('click', function() {
      var options = typeof option === 'object' && option;

      areaSelect = areaSelect && areaSelect.element === element ? areaSelect : new AreaSelect(element, options);
      areaSelect.init();
      if(areaSelect.element.find('.city-select').hasClass("open")) {
        areaSelect.element.find('.city-select').removeClass("open").hide();
        element.find(".aidicon").removeClass("aidicon-chevron-up").addClass("aidicon-chevron-down");

      } else {
        element.find(".aidicon").removeClass("aidicon-chevron-down").addClass("aidicon-chevron-up");
        areaSelect.element.find('.city-select').addClass("open").show();
      }
    });
    return this;
  }
...
+function ($) {
  ...
  Button.prototype.toggle = function () {
    ...
    if ($parent.length) {
      ...
      if (String($input.prop('type')) === 'radio') {
        if ($input.prop('checked')) changed = false;
        $parent.find('.active').removeClass('active');
        this.$element.addClass('active');
      } else if (String($input.prop('type')) === 'checkbox') {
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (String(option) === 'toggle') data.toggle();
...
+function ($) {
  ...
  var Carousel = function (element, options) {
    ...
    String(this.options.pause) === 'hover' && !('ontouchstart' in document.documentElement) && this.$element
  ...
  Carousel.prototype.getItemForDirection = function (direction, active) {
    ...
    var willWrap = (String(direction) === 'prev' && activeIndex === 0)
...
+function ($) {
    ...
    Cascader.prototype.getItemList = function (data, pid) {
        ...
        data.map(function (d) {
            ...
            obj.itemListStyle = self.isListType ? Cascader.EMBELLISHSTYLE.listItem : Cascader.EMBELLISHSTYLE.listDefault;
...
DropdownSelect.prototype.initHead = function () {
    if(this.options.type === 'single'){
        ...
        this.$el.on('click','.dropdownSelect-multi-input-selected .aidicon-close-line',function(){
...
DropdownSelect.prototype.setPosition = function () {
    if(this.options.container){
        ...
        if(String(this.options.panelPos) === 'up'){
            var pos = {};
...
+function ($) {
  ...
  function clearMenus(e) {
    ...
    $(toggle).each(function () {
      ...
      if (e && String(e.type) === 'click' && /input|textarea/i.test(e.target.tagName) && $.contains($parent[0], e.target)) return;
  ...
  Dropdown.prototype.keydown = function (e) {
    ...
    if (!isActive && Number(e.which) !== 27 || isActive && Number(e.which) === 27) {
      if (Number(e.which) === 27) $parent.find(toggle).trigger('focus');
    ...
    if (Number(e.which) === 38 && index > 0)                 index--;         // up
    if (Number(e.which) === 40 && index < $items.length - 1) index++;         // down
    if (!~index)                                    index = 0;
...
+function ($) {
	...
	* */
	...
	// 顶部导航搜索框的鼠标移出事件
	$(document).on("mouseleave", ".navbar .search-icon .form-control", function () {
		var self = $(this);
		var val = $.trim(self.val());
		var isFocus = self.is(":focus");
		if (val.length === 0 && !isFocus) {
			self.closest(".input-group").css("display", "none");
		}
	});
	// 顶部导航搜索框的失去焦点事件
	$(document).on("blur", ".navbar .search-icon .form-control", function () {
		var self = $(this);
		var val = $.trim(self.val());
		if (val.length === 0) {
			self.closest(".input-group").css("display", "none");
	...
	* 头像的悬停事件
	* */
	$(document).on("mouseover", ".avatar.avatar-interaction", function () {
		var self = $(this);
		if (self.find("img").length === 0) {
			self.addClass("avatar-blue-light");
		} else {
			self.addClass("border");
		}
	}).on("mouseout", ".avatar.avatar-interaction", function () {
		var self = $(this);
		var isClick = self.data("click");
		if (self.find("img").length === 0) {
			self.removeClass("avatar-blue-light");
		} else {
			// 如果元素未被点击选中，则图片的鼠标移出事件有效
			if (!!isClick === false) {
	...
	* 头像的点击事件
	* */
	$(document).on("focus", ".avatar.avatar-interaction", function () {
		var self = $(this);
		if (self.find("img").length === 0) {
			self.removeClass("avatar-blue-light").addClass("avatar-blue-week");
		} else {
			self.addClass("border");
			// 设置标记元素已经被点击选中
			self.data("click",true);
		}
	}).on("blur", ".avatar.avatar-interaction", function () {
		var self = $(this);
		if (self.find("img").length === 0) {
			self.removeClass("avatar-blue-week")
		} else {
			self.removeClass("border");
			// 设置标记元素未被点击选中
...
+function ($) {
  ...
  * */
  ...
  /* 时间输入框激活样式修改*/
  +function () {
    $(document).on("focus", ".input-group.input-group-inline.input-group-time .form-control", function () {
      $(this).closest(".input-group-time").addClass("active");
    }).on("blur", ".input-group.input-group-inline.input-group-time .form-control" ,function () {
      $(this).closest(".input-group-time").removeClass("active");
    });
  }();
}(jQuery);
  ...
  Modal.prototype.backdrop = function (callback) {
    ...
    if (this.isShown && this.options.backdrop) {
      ...
      this.$element.on('click.dismiss.bs.modal', $.proxy(function (e) {
        ...
        String(this.options.backdrop) === 'static'
...
+function ($) {
    ...
    NavDrawer.prototype.setPosition = function ($e) {
        var parentPos = $(this.options.container).offset(),
            ...
            pos = {};
        pos.top = String(this.options.container) === 'body' ? (triggerPos.top + this.$ele.outerHeight() + this.options.offset[0] + 0) + "px" : (triggerPos.top - parentPos.top + this.$ele.outerHeight() + this.options.offset[0]) + "px";
        pos.left = (triggerPos.left - parentPos.left + this.options.offset[1]) + "px";
        $e.css(pos);
    }

    NavDrawer.prototype.jumpURL = function (url, target) {
        url && (String(target) === "_blank" ? window.open(url) : window.location.href = url);
    }
...
+function ($) {
    ...
    navTop.prototype.createDoms = function () {
        ...
        for(var i = 0 ; i <this.options.data.length; i++ ){
          ...
          if(list.length <= this.options.limitShow){
            ...
            if (this.options.collapse) {
              modelLiHtml += '<li data-num="'+i+'"  class="list-more"><span>+</span><a>&nbsp;&nbsp;更多</a></li>';
              hoverHtml += '<li data-num="'+i+'"  class="list-collapse"><span>-</span><a>&nbsp;&nbsp;收起</a></li></ul>';
            } else {
              hoverHtml += '</ul>';
            }
          }
    ...
    navTop.prototype.bindEvents = function () {
        ...
        if (String(collapseTrigger) === "click") {
          // 更多按钮的点击事件
...
(function(){
    ...
    Position.advPin = function(pinElement, baseElement, options) {
        ...
        if (String(options.direction) === 'v') { // 垂直定位
        ...
        } else if (String(options.direction) === 'h') { // 水平定位
...
+function ($) {
  ...
  Rate.prototype.show = function () {
    var that = this, html = '';
    if (String(that.options.type) === 'star') { //评分
      html = that.creatStarHtml();
    } else if (String(that.options.type) === 'ticket') { //点赞
      html = that.creatTicketrHtml();
    } else if (String(that.options.type) === 'heart') {  //心形点赞
  ...
  Rate.prototype.creatTicketrHtml = function () {
    ...
    $i.className = 'aidicon aidicon-thumb-' + ((String(that.options.susDirection) === 'up') ? 'up' : 'down') + ((that.options.outline ? '-outline' : '')) + (that.options.disable ? ' disable' : '');
  ...
  Rate.prototype.setGrade = function (grade) {
    if (String(this.options.type) === 'star') {
      this.options.grade = grade;
    } else if (String(this.options.type) === 'ticket') {
  ...
  Rate.prototype.setSelect = function (value) {
    if (String(this.options.type) === 'heart') {
  ...
  Rate.prototype.getGrade = function () {
    ...
    } else if (String(this.options.type) === 'heart') {
      return Number(this.options.isThumb);
    }

  }
  Rate.prototype.amination = function(e, num) {
  ...
  Rate.prototype.goto = function (ele) {
    ...
    if (String(that.options.type) === 'star') {
      if ((parseInt($(ele).index() + 1, 10)) === that.curGrade && that.options.allowClear) {
        $(ele).removeClass('click other').siblings().removeClass('click other');
        that.curGrade = 0;
        that.options.grade = 0;
      } else {
        $(ele).addClass('click').prevAll().addClass('click');
        $(ele).nextAll().removeClass('click other');
        if($(ele).hasClass("rate-half-left")) {
          $(ele).next().nextAll().addClass("other");
        } else {
          $(ele).nextAll().addClass("other");
        }
      ...
      $(ele).trigger(e);
    } else if (String(that.options.type) === 'ticket') {
      var icon = (String(that.options.susDirection) === 'up') ? 'up' : 'down';
      if (String(that.options.sustain) === 'repeal') { // 可撤销的点赞
      ...
      var e = $.Event('change.bs.rate', relatedTarget);
      $(ele).trigger(e);
    } else if (String(that.options.type) === 'heart') {
  ...
  Rate.prototype.hover = function (ele, eventType) {
    var that = this;
    if (String(that.options.type) === 'ticket' || String(that.options.type) === 'heart' || that.options.disable) {
      return;
    }
    if ((parseInt($(ele).index() + 1, 10) === 1) && String(eventType) === 'mouseleave' && that.curGrade === 0) {
      $(ele).removeClass('click');
      $(ele).siblings().eq(that.curGrade).nextAll().removeClass('click');
    } else if ((parseInt($(ele).index() + 1, 10) > that.curGrade) && String(eventType) === 'mouseleave') {
      $(ele).removeClass('click');
...
+function ($) {
  ...
  Step.prototype.init = function () {
    ...
    $.each(that.options.steps, function (index, step) {
      ...
      if (step.state) {
        ...
        if (index === Number(that.options.initStep - 1)) {
          html += ' class="active" ';
        }
      }
        ...
        $li.each(function (index, item) {
          ...
          if ($(item).hasClass("up-down")) {//垂直-上下
            ...
            if (index > 0) {
              ...
              preLiHeight = preLiHeight > 85 ? preLiHeight + 5 : 85;
              ...
              var cssObj = {
                ...
                "height": (preLiHeight * 1 - circleHeight - 8 * 2) + "px",
                "top": (preLiHeight * -1 + circleHeight + 8) + "px"
              };
  ...
  Step.prototype.getExtraStyle = function () {
    ...
    that.$element.find(".progressbar li").each(function () {
      ...
      if (oldLi) {
        var lang = 0;
        if(oldLi.hasClass("small left-right")) {
          lang = oldLi.width() * 0.5 - 12 - oldLi.children("span:first-child").width() - 8 - 6;
        } else {
          lang = oldLi.width() * 0.5 - 16 - oldLi.children("span:first-child").width() - 8 - 6;
        }
        $(this).children(".extra").width(lang);
      }
      oldLi = $(this);
...
+function ($) {
  ...
  Timeline.COllAPSESTYLE = {
    ...
    isEnd: "end",
    endHeadIcon: "aidicon aidicon-check",
    openHeadIcon: "aidicon aidicon-minus",
    closeHeadIcon: "aidicon aidicon-plus"
  }
  // 颜色样式
  Timeline.COLORSTYLE = {
  ...
  Timeline.prototype.getTemplateClass = function (options) {
    ...
    if (sourceDate.length > 0) {
      ...
      sourceDate.map(function (v, i) {
        ...
        obj.tailStyle = String(v.tailStyle) === 'pending' ? that.setTemplateClass(Timeline.MAINSTYLE.tailStyle, Timeline.EMBELLISHSTYLE.pendingStyle) : Timeline.MAINSTYLE.tailStyle;
        ...
        if (options.type === "collapse") {
          ...
          if (v.hasOwnProperty("children") && $childDom.length > 0) {
            ...
            $childDom.map(function (item,index) {
              ...
              tempObj.tailStyle = String(item.tailStyle) === 'pending' ? that.setTemplateClass(Timeline.MAINSTYLE.tailStyle, Timeline.EMBELLISHSTYLE.pendingStyle) : Timeline.MAINSTYLE.tailStyle;
  ...
  Timeline.prototype.getTimelineStyle = function (options) {
    return String(options.type) === 'center' ? true : false;
  }
  ...
  Timeline.prototype.bindEvent = function () {
    $(document).off("click", ".timeline-item-head.timeline-item-head-custom").on("click", ".timeline-item-head.timeline-item-head-custom", function () {
...
+function ($) {
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    for (var i = triggers.length; i--;) {
      ...
      if (String(trigger) === 'click') {
        this.$element.on('click.' + this.type, this.options.selector, $.proxy(this.toggle, this))
      } else if (String(trigger) !== 'manual') {
        var eventIn  = String(trigger) === 'hover' ? 'mouseenter' : 'focusin'
        var eventOut = String(trigger) === 'hover' ? 'mouseleave' : 'focusout'
  ...
  Tooltip.prototype.enter = function (obj) {
    ...
    if (obj instanceof $.Event) {
      self.inState[String(obj.type) === 'focusin' ? 'focus' : 'hover'] = true
    }

    if (self.tip().hasClass('in') || String(self.hoverState) === 'in') {
      self.hoverState = 'in'
      return
    }
    ...
    self.timeout = setTimeout(function () {
      if (String(self.hoverState) === 'in') self.show()
  ...
  Tooltip.prototype.leave = function (obj) {
    ...
    if (obj instanceof $.Event) {
      self.inState[String(obj.type) === 'focusout' ? 'focus' : 'hover'] = false
    }
    ...
    self.timeout = setTimeout(function () {
      if (String(self.hoverState) === 'out') self.hide()
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      if (autoPlace) {
        ...
        if(String(placement) === 'bottom' && pos.bottom + actualHeight > viewportDim.bottom ){
            placement = 'top';
        }
        if(String(placement) === 'bottom-left' ){
            placement = (pos.bottom + actualHeight > viewportDim.bottom) ? 'top-left' :
                        placement
        }
        if(String(placement) === 'bottom-right' ){
            placement = (pos.bottom + actualHeight > viewportDim.bottom) ? 'top-right' :
                        placement
        }
        if(String(placement) === 'top'    && pos.top    - actualHeight < viewportDim.top){
            placement = 'bottom';
        }
        if(String(placement) === 'top-left'){
           placement = (pos.top    - actualHeight < viewportDim.top) ? 'bottom-left' :
                        placement
        }
        if(String(placement) === 'top-right'){
            placement = (pos.top    - actualHeight < viewportDim.top) ? 'bottom-right' :
                        placement
        }
        if(String(placement) === 'right'  && pos.right  + actualWidth  > viewportDim.width){
            placement = 'left';
        }
        if(String(placement) === 'right-top' ){
            placement = (pos.right  + actualWidth  > viewportDim.width) ? 'left-top' :
                        placement
        }
        if(String(placement) === 'right-bottom' ){
            placement = (pos.right  + actualWidth  > viewportDim.width) ? 'left-bottom' :
                        placement
        }
        if(String(placement) === 'left'   && pos.left   - actualWidth  < viewportDim.left ){
            placement = 'right';
        }
        if(String(placement) === 'left-top' ){
            placement = (pos.left   - actualWidth  < viewportDim.left) ? 'right-top' :
                        placement
        }
        if(String(placement) === 'left-bottom' ){
      ...
      var complete = function () {
        ...
        if (String(prevHoverState) === 'out') that.leave(that)
      }
  ...
  Tooltip.prototype.applyPlacement = function (offset, placement) {
    ...
    if (String(placement) === 'top' && actualHeight != height) {
  ...
  Tooltip.prototype.hide = function (callback) {
    ...
    function complete() {
      if (String(that.hoverState) !== 'in') $tip.detach()
  ...
  Tooltip.prototype.getPosition = function ($element) {
    ...
    var el     = $element[0]
    var isBody = String(el.tagName) === 'BODY'
  ...
  Tooltip.prototype.getCalculatedOffset = function (placement, pos, actualWidth, actualHeight) {
    return  String(placement) === 'right-top' ? { top: pos.top,   left: pos.left + pos.width } :
            String(placement) === 'right-bottom' ? { top: pos.top + pos.height  - actualHeight,   left:  pos.left + pos.width } :
            String(placement) === 'left-top' ? { top: pos.top,   left: pos.left - actualWidth } :
            String(placement) === 'left-bottom' ? { top: pos.top + pos.height  - actualHeight,   left: pos.left - actualWidth } :
            String(placement) === 'top-left' ? { top: pos.top - actualHeight,   left: pos.left } :
            String(placement) === 'top-right' ? { top: pos.top - actualHeight,   left: pos.left + pos.width - actualWidth } :
            String(placement) === 'bottom-left' ? { top: pos.top + pos.height,   left: pos.left } :
            String(placement) === 'bottom-right' ? { top: pos.top + pos.height,   left: pos.left + pos.width - actualWidth } :
            String(placement) === 'bottom' ? { top: pos.top + pos.height,   left: pos.left + pos.width / 2 - actualWidth / 2 } :
            String(placement) === 'top'    ? { top: pos.top - actualHeight, left: pos.left + pos.width / 2 - actualWidth / 2 } :
            String(placement) === 'left'   ? { top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left - actualWidth } :
            /* String(placement) === 'right' */ { top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left + pos.width }
