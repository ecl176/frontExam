        ...
        if($.validator){
            ...
            $.validator.setDefaults({
                showErrors: function(errorMap, errorList){
					...
					var $ele = this.currentForm;
					if($($ele).parent().hasClass("bottom-tip")) {
						if(!!errorList.length) {
							for (var i = 0, len = errorList.length; i < len; i++) {
								$(errorList[i].element).next().remove();
								$(errorList[i].element).parent().addClass("has-error");
								$(errorList[i].element).after("<div class='error-tip'><i class='aidicon aidicon-alert-circle'></i>"+errorList[i].message+"</div>");
							}
						} else {
							var len = this.successList.length;
							for (var i = 0; i < len; i++) {
								$(this.successList[i]).next().remove();
								$(this.successList[i]).parent().removeClass("has-error");
							}
						}
					} else {
						if (!!errorList.length) {
